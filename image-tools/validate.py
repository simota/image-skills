#!/usr/bin/env python3
"""Static checks over the image-* skill set.

Every threshold and every piece of vocabulary comes from image-registry/harness.yaml.
...
def load(name: str):
    with (ROOT / "image-registry" / name).open(encoding="utf-8") as fh:
...
def v17_delivery():
    for key, spec in H["delivered"].items():
        want = read(ROOT / "image-registry" / "delivered" / f"{key}.md").rstrip("\n")
        ...
        for d in SKILL_DIRS:
            ...
            if got != want:
                fail("V17", f"{d.name}/SKILL.md {key} block differs from "
                            f"image-registry/delivered/{key}.md (run: make render)")
...
def v28_namespace_agrees():
    ...
    # and a sibling set's install line (`cp -R image-* _common <skills dir>`)
